...
const createPyExample = () => {
    ...
    if (process.platform === 'win32') {
        uvicorn = path.join(home, 'Scripts', 'uvicorn')
        uvicorn += '.exe'
    }
    let port = '' + selectPort()
    ...
    pyProc = require('child_process').spawn(boot, [], { detached: false })
...
const exitPyProc = () => {
    // pyProc.kill()
    //console.log('La pinga')

    // process.kill(-pyProc.pid)
    pyProc = null
    pyPort = null
    let killp = path.join(__dirname, '..', 'kill.bat')
        //pyProc = require('child_process').spawn(killp, [], { detached: true })
        //console.log(killp)
    require("child_process").spawnSync(killp)
    app.quit()
}
